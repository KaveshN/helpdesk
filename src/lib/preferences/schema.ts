 ...
 * `data-density` and `data-sidebar` on <html>, so the first paint is already
 * right and there is no flash or layout shift. A JWT session would also work,
 * but preferences are not identity and should not force a token refresh.
 *
...
export const sidebarSchema = z.enum(['expanded', 'collapsed']);
export type SidebarState = z.infer<typeof sidebarSchema>;

export type Preferences = { theme: Theme; density: Density; sidebar: SidebarState };

export const THEME_COOKIE = 'hd.theme';
export const DENSITY_COOKIE = 'hd.density';
export const SIDEBAR_COOKIE = 'hd.sidebar';
...
export function parseSidebar(raw: string | undefined): SidebarState {
  ...
  return parsed.success ? parsed.data : DEFAULT_PREFERENCES.sidebar;
}
...
export function readPreferences(get: (name: string) => string | undefined): Preferences {
  return {
    theme: parseTheme(get(THEME_COOKIE)),
    density: parseDensity(get(DENSITY_COOKIE)),
    sidebar: parseSidebar(get(SIDEBAR_COOKIE)),
  };
}
